...
public class WordNatureDependencyModel
{
    ...
    static
    {
        ...
        {
            logger.info("加载依存句法生成模型" + MyNLP.Config.WordNatureModelPath + "成功，耗时：" + (System.currentTimeMillis() - start) + " ms");
        }
        else
        {
            logger.warning("加载依存句法生成模型" + MyNLP.Config.WordNatureModelPath + "失败，耗时：" + (System.currentTimeMillis() - start) + " ms");
        }
    }
